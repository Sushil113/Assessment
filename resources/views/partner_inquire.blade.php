...
@section('title', 'Business Partner Inquire')
...
<div class="bg-white shadow rounded p-4">
    <table class="w-full text-left border-collapse">
        ...
        <tbody>
            @foreach ($partnerData as $index => $data)
            <tr class="border-b">
                <td class="p-2">{{ $index + 1 }}</td>
                <td class="p-2">{{ $data->invoice_number }}</td>
                <td class="p-2">{{ $data->company_name }}</td>
                <td class="p-2">{{ $data->contact_name }}</td>
                <td class="p-2">{{ $data->phone_number }}</td>
                <td class="p-2 flex space-x-2">
                    ...
                    <button class="p-2 text-gray-500 hover:text-gray-800 openPersonalInfo">
                        <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
